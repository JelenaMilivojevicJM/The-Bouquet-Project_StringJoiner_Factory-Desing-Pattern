...
public class Main {
	public static void main(String[] args) {
		...
		/*
		 * The following line reads an integer from the user's input and assigns it to
		 * the variable f
		 */
		int f = sc.nextInt();
		...
		/*
		 * The next line creates a new StringJoiner object that will join strings with a
		 * comma as the delimiter.
		 */
		StringJoiner bouquet = new StringJoiner(",");
		if (f == 1) {
			/*
			 * These following lines contain if-else statements that check the value of f
			 * and add the chosen type of flower to the bouquet.
			 */
		 ...
		 * the total number of delimiters will be. So, these following lines calculate
		 * and print the total number of delimiters (which represent leaves in this
		 * context).
		 */
		...
		for (int i = 0; i < totalDelimitersB; i++) {
			// These next lines create an array of leaf emojis and print them out.
			leafs[i] = "🌿";
		}
		for (int i = 0; i < totalDelimitersB; i++) {
		...
		// These following lines calculate and print the total number of flowers in the
		// bouquet.
		...
		/*
		 * This next line closes the Scanner object, indicating that it's no longer
